...
- `npm run test:e2e`: blocked locally because Playwright Chromium executable is unavailable; browser download previously timed out. CI installs Chromium with `npx playwright install --with-deps chromium`. Playwright web server now runs `npm run build` before serving `out/`, so acceptance has no hidden cwd/stale-artifact prerequisite. Smoke assertions cover mocked frontend invoke boundaries, not packaged Tauri runtime logs or live provider traffic.
...
- Passed: `npm run lint`, `npm run typecheck`, `npm test` (14/14), `npm run build`, `cargo fmt --manifest-path src-tauri/Cargo.toml --all -- --check`, `cargo clippy --manifest-path src-tauri/Cargo.toml --all-targets -- -D warnings`, and `cargo test --manifest-path src-tauri/Cargo.toml` (65/65). Custom AI hostnames are resolved before production requests; every resolved loopback/private/link-local/metadata/reserved address rejects, and resolver failures reject. CSV failed-copy cleanup removes destination after import failure. Empty-query parity is documented: backend keeps alphabetical first-five API behavior; UI intentionally requires two characters.
