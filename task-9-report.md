...
- Secret tests use an invocation-scoped generated fixture secret. Rust acceptance sends it through a local mock HTTP server and checks request authentication plus safe request/debug output. Browser smoke retains only redacted invoke metadata and scans project/import bytes and loaded `_next` resources; it cannot observe native runtime logs or a real provider request because Tauri invoke is mocked at browser boundary.
