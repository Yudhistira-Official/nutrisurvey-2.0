...
export default function ReportActions({ request, onMessage }: { request: ExportRequest; onMessage: (message: string) => void }) {
  ...
  const run = async () => { setLoading(true); try { const result = await exportToWord(request); onMessage(result.savedPath ? `Laporan tersimpan: ${result.savedPath}` : 'Laporan berhasil diekspor'); } catch (cause) { onMessage(classifyUiError(cause, 'Export')); } finally { setLoading(false); } };
