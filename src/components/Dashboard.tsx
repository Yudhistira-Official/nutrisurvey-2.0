...
export default function Dashboard({ foods, meals, targets, onAddMeal, onAddFood, onRemoveMeal, onRemoveFood, onAmount, onMoveFood }: {
  ...
  return <section>
    ...
    <div className="dashboard-grid">
      <div className="card table-card"><h3>Daftar Konsumsi</h3><table><thead><tr><th>Nama Makanan</th><th>Gram</th><th>kcal</th><th>Carb</th><th>Prot</th><th>Fat</th><th /></tr></thead>
        {meals.map(meal => <tbody key={meal.id} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); const foodId = event.dataTransfer.getData('text/plain'); if (foodId) onMoveFood(foodId, meal.id); }}><tr className="meal-header"><td colSpan={7}><strong>{meal.label}</strong><span><button onClick={() => onAddFood(meal.id)}>+</button><button onClick={() => onRemoveMeal(meal.id)}>×</button></span></td></tr>
          {foods.filter(food => food.mealTime === meal.id).map(food => { const ratio = toNumber(food.amount) / (toNumber(food.servingSize) || 100); return <tr key={food.id} draggable onDragStart={event => event.dataTransfer.setData('text/plain', food.id)}><td>⋮⋮ {food.name}</td><td><input type="number" value={food.amount} onChange={event => onAmount(food.id, toNumber(event.target.value))} /> {food.servingUnit}</td><td>{(getMacroValue(food.nutrients, 'energy') * ratio).toFixed(0)}</td><td>{(getMacroValue(food.nutrients, 'carbs') * ratio).toFixed(1)}</td><td>{(getMacroValue(food.nutrients, 'protein') * ratio).toFixed(1)}</td><td>{(getMacroValue(food.nutrients, 'fat') * ratio).toFixed(1)}</td><td><button className="danger" onClick={() => onRemoveFood(food.id)}>×</button></td></tr>; })}</tbody>)}
      </table></div>
